...
from C_MainScripts.EpochPerformance import EpochPerformance
...
def callbacks_list(CM_train_generator, CM_validation_generator, labels, results_dir):
    ...
    # if task = cn-mci-ad
    #filepath = results_dir + "/weights-improvement-{epoch:02d}-{val_acc:.2f}.hdf5" # change to mauc??
    filepath = results_dir + "/weights-improvement-{epoch:02d}-{mauc:.2f}.hdf5"

    acc_checkpoint = keras.callbacks.ModelCheckpoint(filepath, monitor='val_acc', verbose=1, save_best_only=True,
                                                     save_weights_only=True, mode='auto', period=1)

    # if task = cn-mci-ad
    #acc_checkpoint = keras.callbacks.ModelCheckpoint(filepath, monitor='mauc', verbose=1, save_best_only=True,
    #                                                 save_weights_only=True, mode='max', period=1) # monitor = mauc and mode = 'max'
